...
EtherChannel combines multiple physical links into a single logical link called a **Port-Channel**. This provides increased bandwidth, redundancy, better link utilization, and network resiliency.

In this lab, four FastEthernet links are bundled together using LACP and configured as a single logical **Port-Channel 1**. The Port-Channel is configured as a trunk link between the two switches.

A physical link failure test is also performed to verify that network connectivity continues through the remaining EtherChannel member links.
...
Both PCs are configured in the same VLAN and subnet. Therefore, a default gateway is not required for this same-network connectivity test.

---

# ⚙️ Configuration

## 1. Configure VLAN 10 and Access Port on SW0
...
## 2. Configure VLAN 10 and Access Port on SW1
...
Both switches use **LACP active mode** to dynamically negotiate and form the EtherChannel.
...
The four physical links now operate as a single logical **Port-Channel 1**.
...
If one physical link fails, the remaining EtherChannel member links continue forwarding traffic.
...
I also performed a physical link failure test and verified that network connectivity continued through the remaining EtherChannel links. This demonstrated the redundancy, resiliency, and link-failure recovery provided by EtherChannel.
